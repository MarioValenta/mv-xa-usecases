...
import { getDateTimeLocal, getDateTimeUTC, getFlatpickrSettings } from 'projects/shared.functions';
...
@Component({
  selector: 'universal-tap-execution-bgt-win-poststartup-legacy-request-form',
  templateUrl: './app.component.html',
  styleUrls: ['./app.component.css']
})
export class AppComponent extends RequestContextBaseComponent implements ICERequest {

  title = 'BGT-PostStartup Deployment - Windows Legacy';
  startOptions: FlatpickrOptions = getFlatpickrSettings({minDate: "today"});
  ...
  get hostControl(): FormControl {
    return this.form.get([this.FORM_JENKINS_PARAMETERS.key, this.FORM_HOST.key]) as FormControl;
  }
  ...
  customOnInit(): void {
    ...
    this.dataService
      ...
      .subscribe(
        ...
        // set initial startDate value with current date time local
        this.startDateControl.setValue(getDateTimeLocal(0));
  }
  ...
  buildForm() {
    ...
    this.form = this.fb.group({
      ...
      [this.FORM_JENKINS_PARAMETERS.key]: this.fb.group({
        [this.FORM_HOST.key]: ['']
      })
    });
  }
