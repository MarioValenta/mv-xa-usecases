...
export class DataService {
  ...
  // Get KMS for Customer
  public getKMSFor(customer: string){
     return this.xaservices.Http!.Get<any>(`api/vmcreate/kms/${customer}`).pipe(
       share()
     );
  }
  ...
  // Get admin LAN customer
  public getAdminLANCustomer(customer: string){
    return this.xaservices.Http!.Get<Array<any>>(`/api/vmcreate/adminlan/${customer}`).pipe(
      share()
    );
  }
  ...
  // Get LANs for the provided customer with specific LAN ID
  public getLANsForCustomerWithCustomerLANId(customer: string, id: number){
    return this.xaservices.Http?.Get<Array<any>>(`/api/cmdb/lan/${customer}/?id=${id}`).pipe(
      share()
    );
  }

  // Get NAS LANs for the provided customer
  public getNASLAN(customer: string, FkNasNetId: number) {
    return this.xaservices.Http!.Get<Array<any>>(`/api/cmdb/lan/${customer}/?id=${FkNasNetId}`).pipe(
      share()
    );
  }
